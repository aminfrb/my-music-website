...
function UploadContent() {
  ...
  // Set when the server rejects the upload as a song that already exists —
  // rendered as an introduction to the original uploader rather than a red bar.
  ...
  // Instant feedback on obviously-generated names; the server has the final say.
  const titleLooksAuto = looksAutoGenerated(meta.title);
  const artistLooksAuto = looksAutoGenerated(meta.artistName);
  ...
  const fail = (err: unknown) => {
    if (err instanceof GraphQLError) {
      ...
      const dup = err.details?.duplicate as DuplicateDetails | undefined;
  ...
  const onAudioSelected = async (file: File) => {
    ...
    try {
      ...
      // Deliberately *not* pre-filling the title from the file name — that is
      // how tracks end up called "8432" or "AUD-20240115-WA0007". The user
      // types the real name, and the server enforces that it is one.
      setStep(1);
    } catch (err) {
      fail(err);
      setAudioName("");
      ...
      <div className="rounded-card border border-border bg-surface/40 p-5">
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <Field
              label={t("title")}
              hint={titleLooksAuto ? t("nameLooksAuto") : t("titleHint")}
            >
              <Input
                value={meta.title}
            ...
            <Field
              label={t("artistName")}
              hint={artistLooksAuto ? t("nameLooksAuto") : t("artistNameHint")}
            >
              <Input
